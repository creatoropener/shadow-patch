    ...
    if requested in (None, "node-typescript") and (root / "package.json").is_file() and (root / "tsconfig.json").is_file():
        ...
        return RuntimeAdapter(
            ...
            verifier_guidance=(
                ...
                "Keep stream creation, pipeThrough composition and collectBytes consumption "
                "inside that async callback, not before it. Keep imports and unrelated setup "
                "outside the assertion. A later equality assertion cannot catch an earlier rejection. "
